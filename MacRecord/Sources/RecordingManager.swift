...
final class SampleWriter: @unchecked Sendable {
    ...
    func setUp(url: URL, videoWidth: Int, videoHeight: Int, includeMicrophone: Bool = false) throws {
        ...
        if includeMicrophone {
            // Mic format hint — mono float32 48kHz to match typical mic device format
            ...
            var micSourceFormat: CMAudioFormatDescription?
            CMAudioFormatDescriptionCreate(
                allocator: kCFAllocatorDefault,
                asbd: &micAsbd,
                layoutSize: 0,
                ...
                magicCookie: nil,
                extensions: nil,
                formatDescriptionOut: &micSourceFormat
            )
            ...
            let mInput = AVAssetWriterInput(mediaType: .audio, outputSettings: micSettings, sourceFormatHint: micSourceFormat)
    ...
    func appendMicSampleBuffer(_ sampleBuffer: CMSampleBuffer) {
        ...
        // Same pause adjustment as video/audio — SCStream mic shares the same time base
        let adjustedTime = CMTimeSubtract(timestamp, totalPausedTime)

        if firstMicTimestamp == nil {
            firstMicTimestamp = adjustedTime
            DebugLog.shared.send("[Writer] First mic sample at \(adjustedTime.seconds)s")
        }
...
final class StreamOutput: NSObject, SCStreamOutput, SCStreamDelegate, @unchecked Sendable {
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
        if type == .microphone {
            writer?.appendMicSampleBuffer(sampleBuffer)
        } else {
